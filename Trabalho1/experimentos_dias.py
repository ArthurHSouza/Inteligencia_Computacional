...
def run_optimization(r_f, rho_max, initial_population=40, gen=30, tournsize=3, mprob=0.2, max_days=100):
...
for i in range(0, 200):
    ...
    log_geo = run_optimization(r_f_far, 1000.0,
                               initial_population=ini_pop, gen=gen, tournsize=tournsize, max_days=100)
    # Cenário 1: LEO para GEO [cite: 179]
    #log_geo = run_optimization(42164 * u.km, 40.0,
    #                       initial_population=ini_pop, gen=gen, tournsize=tournsize, mprob=mprob)
    rb_local = log_geo[0]
    ga_local = Maneuver.bielliptic(ORB_I, rb_local * u.m, r_f_far)
    medias_geo_rb = numpy.append(medias_geo_rb, rb_local)
    medias_geo_deltav = numpy.append(medias_geo_deltav, ga_local.get_total_cost().to(u.m/u.s).value)
